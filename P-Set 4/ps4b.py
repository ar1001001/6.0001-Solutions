...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for i in range(len(alphalower)):
            if (i + shift) < 26:
                ...
                alphalower[i] = alphalower1[i + shift - 26]
        ...
        for i in range(len(alphalower)):
            if (i+shift) <26:
                ...
                alphaupper[i] = alphaupper1[i + shift-26]
    ...
    def apply_shift(self, shift):
        ...
        Returns: the message text (string) in which every character is shifted
             ...
             :return:
        '''
        cipdict = self.build_shift_dict(shift)
        s = list(self.message_text)
        for i in range(len(s)):
                s[i] = cipdict.get(s[i], s[i])
        return ''.join(s)
...
class PlaintextMessage(Message):
    def __init__(self, text, shift):
        ...
        self.text_encrypted = self.apply_shift(shift)
        self.shift = shift
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        '''
        words = self.message_text
        listx = list(words.split(' '))
        best_i = 0
        valid_words = 0
        phrase_final = ''
        for i in range(26):
            s = 26 - i
            valid_words_s = 0
            listx = list(self.apply_shift(s).split(' '))
            for u in listx:
                if is_word(wordlist, u):
                    valid_words_s += 1
            if valid_words_s >= valid_words:
                valid_words = valid_words_s
                phrase_final = ' '.join(listx)
                best_i = s
        return (best_i, phrase_final)
